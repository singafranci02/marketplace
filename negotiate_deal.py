...
# Phase 32/33: autonomous Solana escrow signing (graceful if solders not installed)
try:
    from solana_agent import execute_lock_and_commit as _execute_lock_and_commit, SOLDERS_AVAILABLE
...
def generate_signed_artifact(
    ...
    # Phase 32/33: autonomously lock funds on-chain with artifact hash commitment
    if _SOLANA_ENABLED:
        try:
            seller_sol_pubkey = artifact.parties["licensor"].get("solana_pubkey")
            if seller_sol_pubkey:
                price_sol     = artifact.terms.get("rev_share_pct", 0) / 100
                artifact_hash = hashlib.sha256(body).hexdigest()  # hash of signed canonical body
                print(f"\n[SOLANA] Submitting lock_and_commit for {price_sol:.4f} SOL...")
                tx_sig = _execute_lock_and_commit(
                ...
                _log("lock_and_commit_submitted", {
                    "artifact_id":   artifact.artifact_id,
                    "artifact_hash": artifact_hash,
                    "tx_hash":       tx_sig,
                })
        except Exception as exc:  # noqa: BLE001
            print(f"\n[SOLANA] lock_and_commit skipped: {exc}")
    else:
